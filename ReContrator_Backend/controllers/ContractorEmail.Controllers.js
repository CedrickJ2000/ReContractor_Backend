...
exports.signup = async (req, res) => {
    try {
      ...
      const userrandomID = await randomstring.generate();
      ...
      if (findEmail.length >= 1 && findUserName.length >= 1) {
        ...
        const data = {
          from: 'ReContractor@gmail.com',
          to: EmailAddress,
          subject: 'Account Activation Link',
          html:`
          <h3>Please click on given link to activate your account</h3>
          <h2>This will be your Contractor ID </h2><h1>${userID}</h1>
  ...
  exports.activateAccount =  (req, res) => {
   ...
   if(token){
      jwt.verify(token, process.env.JWT_ACC_ACTIVATE, function(err, decodedToken){
        ...
        const { UserName, FirstName, LastName, EmailAddress, Password, Address, userID } = decodedToken;
        Contractor.findOne({EmailAddress, userID}).exec((err, contractor) =>{
          if(contractor){
            return res.status(400).json({error: "User with this email is already exists."});
          }
          
          let newContractor = new Contractor({UserName, FirstName, LastName, EmailAddress, Password, Address, userID });
          newContractor.save((err, success) => {
            ...
            res.json({
              message: "Account Activation Success! Please check the previous email for your Contractor ID"
